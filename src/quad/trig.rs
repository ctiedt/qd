...
impl Quad {
    ...
    pub fn atan2(self, other: Quad) -> Quad {
        match self.pre_atan2(&other) {
            ...
            None => {
                ...
                let mut z = Quad::from(libm::atan2(self.0, other.0));
    ...
    /// absolute value. `x.atan()` is the same as `x.atan2(Quad::ONE)`.
    ///
    /// The domain of this function is $(-\infin, \infin)$ and the range is
    /// $[-\frac{\pi}{2}, \frac{\pi}{2}]$.
    ///
    ...
    ///
    /// [`atan2`]: #method.atan2
    pub fn atan(self) -> Quad { self.atan2(Quad::ONE) }
...
fn reduce(a: Quad) -> (i32, i32, Quad) {
    ...
    // reduce modulo π/2
    let mut q = libm::floor(r.0 / Quad::FRAC_PI_2.0 + 0.5);
    let mut t = r - Quad(q, 0.0, 0.0, 0.0) * Quad::FRAC_PI_2;
    let j = q as i32;

    // reduce modulo π/1024
    q = libm::floor(t.0 / FRAC_PI_1024.0 + 0.5);

...
mod macros {
    ...
    /// `Double`, `Quad`, any primitive number, and 2-, 3-, and 4-tuples of
    /// any of those primitive number types.
    ///
    /// # Examples
    /// ```
    /// # use qd::{qd, Quad};
...
/// A 256-bit floating-point number implemented as the unevaluated sum of four
/// 64-bit floating-point numbers. Discarding the bits used for exponents, this
/// makes for about 212 bits of mantissa accuracy, or around 63 decimal digits.
///
...
/// arguments must be pre-normalized. [`from`], [`parse`], and [`qd!`] will both
...
/// without rounding). In this case, [`from`] and [`qd!`] can also skip
...
impl Quad {
    ...
    /// normalization is obviously unnecessary. For example, if a `Quad`
    /// version of the number `10` is needed, `Quad::new(10.0, 0.0, 0.0,
    /// 0.0)` is a good way to do it in order to save the cost of the
    /// normalization that is obviously not needed.
    ///
    /// # Examples
    /// ```
    /// # use qd::Quad;
    ...
    /// assert!(q.is_zero());
    /// ```
    pub const fn new(a: f64, b: f64, c: f64, d: f64) -> Quad { Quad(a, b, c, d) }
}
...
impl Index<usize> for Quad {
    ...
    ///
    /// Using index `0` will return the first component, using index `1` will
    /// return the second, and so on.
    ///
    /// One capability that is *not* provided is mutable indexing; ensuring that
    /// a `Quad` is normalized would be impossible if they could be
    /// individually changed at will. `Quad`s are immutable like any other
    /// number; if you need a new value for a `Quad`, you should simply
    /// create a new `Quad`.
    ///
    ...
    /// component of a `Quad` other than the first.Quad
    ///
    /// # Examples
    /// ```
    /// # use qd::Quad;

...
impl Double {
    ...
    /// result is too large to represent with a `Double`; at that point the
    /// function begins to return [`INFINITY`]. The limit on the low end is
    /// less due to the fact that the second component needs to fit in an
    /// `f64` rather than the first, along with extra bits used in argument
    /// reduction; this function begins to return 0 at -600.
    ///
    /// As $x$ grows this function does lose a bit of precision. It's precise to
    /// at least 30 digits up to values of $-140 \le x \le 150$, and from
    /// then until the limits, it's precise to at least 29 digits.
    ///
    /// # Examples
    /// ```
    /// # use qd::dd;
    ...
    pub fn exp(self) -> Double {
        match self.pre_exp() {
            ...
            None => {
                ...
                // arithmetic.
                let k = libm::floor(self.0 / Double::LN_2.0 + 0.5);
    ...
    /// -600, the accurate results of this function are limited to the
    /// number whose logarithm is 600, which is around `2.65e261`. Take care
    /// with this; unlike in [`exp`], [`INFINITY`] is *not* returned. In
    /// that function, exceeding the maximum refers to actually overflowing
    /// an `f64`, which is appropriate to call [`INFINITY`]; here, it means
    /// `601`.
    ///
    /// # Examples
    /// ```
    ...
    pub fn ln(self) -> Double {
        match self.pre_ln() {
            ...
            None => {
                ...
                let mut x = Double(libm::log(self.0), 0.0); // initial approximation
    ...
    /// numbers themselves. In this case, that upper limit is around
    /// `1e261`. Over this number, the output is not reliable, but it does
    /// not return [`INFINITY`] because the number 261 is so plainly not
    /// infinite.
    ///
    /// # Examples
    /// ```
    ...
    /// [`ln`]: #method.ln
    #[inline]
    pub fn log10(self) -> Double { self.ln() / Double::LN_10 }
    ...
    #[inline]
    pub fn log2(self) -> Double { self.ln() / Double::LN_2 }
